...
if (sequence_type == "16S"){
    rownames(counts) <- gsub("__-", "unassigned", gsub("_NA", "unassigned", gsub(";", "\\|", rownames(counts))))
...
ggplot(known.ge.m, aes(x=variable, y=value, fill=genus)) + geom_bar(stat="identity") + theme_bw() + scale_fill_manual(values=colours.ge) + xlab("") + ylab("% reads")
...
p <- ggplot(dat, aes_string(x=sample, y="Expected")) + geom_point() + theme_bw() + stat_smooth(method="lm") + geom_label_repel(label=dat$species)
grobs.sp[[i-3]] <- p
}
...
p <- ggplot(dat, aes_string(x=sample, y="Expected")) + geom_point() + theme_bw() + stat_smooth(method="lm") + geom_label_repel(label=dat$genus)
grobs.ge[[i-3]] <- p
}
