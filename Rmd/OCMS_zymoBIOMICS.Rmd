...
##############################################################################
##############################################################################
# function to plot out fraction of reads that are false positive and true positives in each sample
# count-table = dataframe of counts at the level of interest
# annotations = dataframe of true positives 
# tax_level = one of c('genus','species')
false_pos_count <- function(count_table, annotations, tax_level) {
  
  # prepare plot data
  pdata <- melt(count_table) %>%
    dplyr::mutate(tax_status = ifelse(.data[[tax_level]] %in%
                                        annotations[,tax_level],
                                      "TP","FP"))
  p <- ggplot(pdata, aes(x = variable, y = value)) +
    geom_bar(stat = 'identity', 
             aes_string(fill = 'tax_status', colour = tax_level)) +
    scale_colour_manual(values = rep('black', length(unique(pdata[,tax_level]))),
                        guide = FALSE) +
    scale_fill_manual(values=c('grey','purple')) +
    xlab('Samples') +
    ylab(sprintf("Read count, %s", tax_level)) +
    theme_classic(12) +
...
################################################################################
################################################################################
# function to list out taxa observed, ranked by abundance and coloured as TP or FP. produces at DT::datatable
# relab_table = relative abundance table aggregated at level of interest
# rank_table = output of truePosRate
# annotations = dataframe of true positives
# tax_level = one of c("genus","species")
rank_tpfp <- function(rank_table) {
  
  out <- DT::datatable(rank_table[,c('sample','rank','taxon','abundance',
                                'true.pos.rate','label')],
                       selection = 'none',
                       rownames = FALSE,
                       filter='top',
                       options = list(order=list(list(0, 'asc'),
                                                 list(1,'asc')))) %>%
    DT::formatRound(columns=c('true.pos.rate', 'abundance'), digits=1) %>%
    DT::formatStyle('rank', 'label',
                color = DT::styleEqual(c('TP','FP'),c('purple', 'grey'))) %>%
    DT::formatStyle('taxon', 'label',
                color = DT::styleEqual(c('TP','FP'),c('purple', 'grey'))) %>%
    DT::formatStyle('abundance', 'label',
                color = DT::styleEqual(c('TP','FP'),c('purple', 'grey'))) %>%
    DT::formatStyle('true.pos.rate', 'label',
                color = DT::styleEqual(c('TP','FP'),c('purple', 'grey'))) %>%
    DT::formatStyle('label',
                color = DT::styleEqual(c('TP','FP'),c('purple', 'grey')))
...
`r if("species" %in% show_levels){"## Species"}`

```{r tp.rate.se, eval="species" %in% show_levels, echo=FALSE, message=FALSE, fig.height=5, fig.width=10}
...
`r if("genus" %in% show_levels){"## Genus"}`

```{r tp.rate.ge, eval="genus" %in% show_levels, echo=FALSE, message=FALSE, fig.height=5, fig.width=10}
...
```{r composition.species, eval="species" %in% show_levels, echo=FALSE, message=FALSE, fig.height=5, fig.width=7}
...
```{r composition.genus, eval="genus" %in% show_levels, echo=FALSE, message=FALSE, fig.height=5, fig.width=7}
...
```{r}
fig_height = ceiling(length(standards_columns)/2) * 4
```

```{r correlation.species, eval="species" %in% show_levels, echo=FALSE, message=FALSE, fig.height=fig_height, fig.width=10}
grobs.sp <- list()
...
```{r correlation.genus, eval="genus" %in% show_levels, echo=FALSE, message=FALSE, fig.height=fig_height, fig.width=10}
grobs.ge <- list()
...
```{r pca.sp, eval="species" %in% show_levels, echo=FALSE, message=FALSE, fig.height=5, fig.width=6}
...
```{r pca.ge, eval="genus" %in% show_levels, echo=FALSE, message=FALSE, fig.height=5, fig.width=6}
...
```{r sp.fp.bar, eval="species" %in% show_levels}
toplot.sp <- counts
species <- unlist(strsplit(rownames(toplot.sp), "\\|"))
species <- gsub("s__", "", species[seq(7, length(species),7)])
toplot.sp <- aggregate(toplot.sp, list(species), sum)
...
false_pos_count(toplot.sp, annotations, 'species')
```

```{r sp.fptp, eval="species" %in% show_levels}
rank_tpfp(tp.rate.sp)
```

`r if("genus" %in% show_levels){"## Genus"}`

```{r ge.fp.bar, eval="genus" %in% show_levels}
toplot.ge <- counts
genus <- unlist(strsplit(rownames(toplot.ge), "\\|"))
genus <- gsub("g__", "", genus[seq(6, length(genus),7)])
toplot.ge <- aggregate(toplot.ge, list(genus), sum)
...
false_pos_count(toplot.ge, annotations, 'genus')
```

```{r ge.fptp, eval="genus" %in% show_levels}
rank_tpfp(tp.rate.ge)
```
