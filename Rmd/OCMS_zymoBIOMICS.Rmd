...
truePosRate <- function(relab, annotations, level="species", cutoff=0.01){
    ...
    if (level == "species"){
        shortnames <- get_shortnames(rownames(relab))
        ...
        shortnames <- gsub(";s__.*", "", rownames(relab))
        shortnames <- get_shortnames(shortnames)
	    }
...
# don't read with rownames as problematic for kraken2
counts <- read.csv(counts_table, header=TRUE, stringsAsFactors=FALSE, sep="\t")

# If it's 16S data then need to sub the ";" with "|" and change NA to unassigned
if (sequence_type == "16S"){
    rownames(counts) <- counts[,1]
    counts <- counts[,2:ncol(counts)]
    rownames(counts) <- gsub("\\(.*", "", rownames(counts))}else{
    # aggregate kraken2
    counts_to_agg <- counts[,2:ncol(counts)]
    counts <- aggregate(counts_to_agg, by=list(counts[,1]), FUN="sum")
...
species <- get_shortnames(rownames(relabundance))
...
genus <- gsub(";s__.*", "", rownames(toplot.ge))
genus <- get_shortnames(genus)
